...
| Exec / silent walkthrough of the same path | `helix-walkthrough.mp4` |
...
## 10. Cloud messaging and functions (SNS, SQS, Lambda, Event Grid, Service Bus)

These are **transports and glue**, not a close.

| Ask | Answer |
|---|---|
| **Why not SNS + SQS?** | Fan-out and queues. A receipt is not “R-2031 BLOCKED on Motif MB014”. You still write the fold, command-once, CEES 404, audit, console. |
| **Why not Lambda / Azure Functions / Step Functions?** | You would re-implement this hub as a pile of functions. No event store replay, no fail-closed instance, no onboard-as-data, burst cost at COB, data leaves the bank. |
| **Why not EventBridge / Event Grid?** | Same category as Kafka: they move facts. They do not decide Ready. |
| **Why not Amazon MQ / Azure Service Bus?** | Same as IBM MQ: a good **adapter behind** `POST /api/events`. Not the product. |

Cloud also fails the on-prem pitch: close keys, run ids, and sign-off evidence stay **in the bank’s DC**. Hyperscaler spend at COB burst is the wrong bill for a regulated fold.

---

## 11. Redis (and “do we need a cache?”)

**This application does not use Redis.** The fold is event-sourced. State rebuilds from the event store (H2 now, Oracle later).

| Temptation | Why not |
|---|---|
| Cache Ready/Blocked in Redis | Second truth. Restart / failover shows a colour the event store does not own. |
| Cache Reports meters | Optional later for read scale. Never the system of decision. |
| Session / entitlement cache | CEES is fail-closed. A warm cache must not leak a 200 for an unentitled row. |

Phase 1: **no Redis, no Kafka, no licence.** Three processes. HTTP ingest. SSE to the browser.

---

## 12. Use the application as-is (cheapest path that still works)

Yes. That is the POC you can run in a room today.

| Piece | Phase 1 (as-is) | Later, only if the bank already owns it |
|---|---|---|
| Ingest | `POST /api/events` | Thin adapter from Kafka / Solace / IBM MQ / CDC |
| Fold + command + audit + board | **This hub** | Unchanged |
| Browser | REST + SSE | Unchanged — never Kafka in Chrome |
| Cache | None | Redis only as a read cache, never the fold |
| Batch | None | Airflow / Control-M **after** READY |
| Persistence | H2 file | Oracle already on the estate |

You do **not** need to buy AWS, Azure, Kafka, or Redis to prove or to run the first outcomes. Origins that can POST a fact (or a 20-line adapter) are enough. Kafka/Redis are **optional later**, behind the same API — they do not replace the product.

---

## 13. Why on-prem wins this product

| On-prem (this plan) | Hyperscaler / paid “control tower” |
|---|---|
| Facts and sign-off stay in the bank | Egress, residency, and a second control plane |
| Reuse Kafka/MQ/Oracle **already paid** | New SNS/SQS/Lambda/Fabric bill at close burst |
| Three JVM processes on existing estate | New cloud account, IAM, PrivateLink, DR in someone else’s region |
| Next outcome is data (`POST /definitions`) | New functions, new DAGs, new dashboards per product |
| Partner screens stay iframes | Vendor copies Motif/Helix data into their lake |

**Pitch line:** we are not asking for a platform programme. We are asking to standardise a thin, already-working fold on hardware and licences the bank already has — and to refuse N shadow folds in Airflow, Lambda, and Power BI.
